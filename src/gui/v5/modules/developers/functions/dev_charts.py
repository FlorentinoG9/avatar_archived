...
# Refactored: Moved to gui/v5/modules/developers for better organization
...
def run_shortlog_all() -> list[tuple[str, int]]:
    """
    Runs git shortlog to get commit counts per author.
    Returns list of (author_name, commit_count) tuples sorted by commits (desc).
    """
    proc = subprocess.run(
...
def assign_fixed_tiers(data: list[tuple[str, int]]) -> list[tuple[str, int, str]]:
    """
    Assigns Gold/Silver/Bronze tiers to top 15 contributors.
    Returns list of (author_name, commit_count, tier).
    """
    top_15 = data[:15]
...
def save_csv(rows: list[tuple[str, int, str]], outpath: str):
    """Saves tiered contributor data to CSV."""
...
def devList() -> List[str]:
    """
    Returns a list of all developer names from git shortlog.
    Excludes specific authors defined in exclude set.
    """
    try:
        proc = subprocess.run(
            ["git", "shortlog", "-sne", "--all"],
...
def plot_single_tier(rows: list[tuple[str, int, str]], tier: str, outpath: str):
    """
    Creates a bar chart for a specific tier (Gold/Silver/Bronze).
    Saves to outpath as PNG.
    """
...
def main():
    """
    Main entry point for generating contributor analytics.
    Generates CSV and PNG charts for Gold/Silver/Bronze tiers.
    """
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory to save outputs (defaults to gui/v5/modules/developers/output).",
    )
    args = parser.parse_args()

    # Default output to GUI assets directory for easy QML access
    if args.out_dir is None:
        module_dir = Path(__file__).parent
        gui_assets_dir = module_dir.parent.parent / "assets" / "developers"
        args.out_dir = str(gui_assets_dir)
