...
def create_train_state(rng, input_dim, hidden_dims, n_classes):
    ...
    model = MLPClassifier(
        hidden_dims=hidden_dims, n_classes=n_classes, dropout_rate=DROPOUT_RATE
    )
    ...
    tx = optax.chain(
        optax.clip_by_global_norm(GRADIENT_CLIP_NORM), optax.adam(schedule)
    )
...
def train_model(X_train, y_train, X_val, y_val, n_epochs, batch_size):
    ...
    print(
        f"Architecture: Input({X_train.shape[1]}) -> {HIDDEN_DIMS} -> Output({n_classes})"
    )
    ...
    state = create_train_state(init_rng, X_train.shape[1], HIDDEN_DIMS, n_classes)

    history = {"train_loss": [], "train_acc": [], "val_acc": [], "epochs": []}
...
def plot_training_history_comprehensive(history, output_path):
    ...
    history_path = os.path.join(
        output_path, "training_history_comprehensive_deep_learning.png"
    )
...
def plot_confusion_matrix_enhanced(y_true, y_pred, class_names, output_path):
    ...
    ax.set_title(
        "Confusion Matrix - JAX Deep Learning\nNAO Robot EEG Control",
        fontsize=16,
        fontweight="bold",
    )
...
def main():
    ...
    try:
        ...
        state, best_acc, history = train_model(
            X_train, y_train, X_val, y_val, N_EPOCHS, BATCH_SIZE
        )
