...
class Brainwaves:
    ...
    def __init__(self, get_drone_action):
        self.signal_system = sgsys(
            get_drone_action, "holding pattern"
        )  # The signal system class

    keep_alive_toggle = False  # mimics the drone's keep alive toggle

    # Layout
    flight_log: list[str] = []  # array to hold flight log info
    predictions_log: list[list[str]] = []  # array to hold info for table
    ...
    def brainwave_prediction_window(self, get_drone_action, use_brainflow):
        top_left = [
            ...
            [
                sg.Button(
                    "Read my mind...", size=(40, 5), image_filename="GUI_Pics/brain.png"
                )
            ],
            ...
            [
                sg.Table(
                    values=[],
                    headings=self.response_headings,
                    ...
                    key="-SERVER_TABLE-",
                    row_height=25,
                    tooltip="Server Response Table",
                    hide_vertical_scroll=True,
                )
            ],
            [
                sg.Button("Not what I was thinking...", size=(14, 3)),
                sg.Button("Execute", size=(14, 3)),
                sg.Push(),
            ],
            [sg.Input(key="-drone_input-"), sg.Button("Keep Drone Alive")],
        ]
        bottom_left = [
            [sg.Text("Flight Log")],
            [sg.Listbox(values=self.flight_log, size=(30, 6), key="LOG")],
        ]
        bottom_right = [[sg.Text("Console Log")], [sg.Output(s=(45, 10))]]

        brainwave_prediction_layout = [
            [
                sg.Column(top_left, pad=((150, 0), (0, 0))),
                sg.Push(),
                sg.Table(
                    values=[],
            ...
            [
                sg.Button(
                    "Connect", size=(8, 2), image_filename="GUI_Pics/connect.png"
                ),
                sg.Push(),
            ],
        ]
        ...
        tab = sg.Tab(
            "Brainwave Reading", brainwave_prediction_layout, key="Brainwave Reading"
        )
        return tab
    ...
    def buttonLoop(self, window1, event1, values1, get_drone_action, use_brainflow):
        ...
        if event == "Read my mind...":
            ...
            window1["-SERVER_TABLE-"].update(values=server_record)
            ...
            window1["-TABLE-"].update(values=self.predictions_log)
        elif event == "Execute":
