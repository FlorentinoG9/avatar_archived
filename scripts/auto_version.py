...
class ConventionalCommit:
    ...
    def get_version_bump(self) -> str:
        ...
        if self.breaking_change:
            return "major"
        return str(self.TYPE_MAPPING.get(self.type, "patch"))

    def get_changelog_section(self) -> str:
        """Get appropriate changelog section"""
        return str(self.CHANGELOG_MAPPING.get(self.type, "Changed"))
    ...
    def format_for_changelog(self) -> str:
        ...
        return (
            f"- {self.description.capitalize()}{scope_text}{breaking_text}{hash_text}"
        )
...
class VersionManager:
    ...
    def get_commits_since_last_release(self) -> List[git.Commit]:
        ...
        try:
            ...
            for commit in commits:
                if (
                    not commit.message.startswith("Merge")
                    and "[skip ci]" not in commit.message
                    and not commit.message.startswith("chore: release")
                ):
    ...
    def bump_version(
        self, current_version: semantic_version.Version, bump_type: str
    ...
    def update_changelog(
        self, new_version: semantic_version.Version, entries: Dict[str, List[str]]
    ):
    ...
    def create_release_notes(
        self, version: semantic_version.Version, entries: Dict[str, List[str]]
    ):
        ...
        release_notes += "---\n\n"
        release_notes += "**Full Changelog**: [CHANGELOG.md](https://github.com/3C-SCSU/Avatar/blob/main/CHANGELOG.md)\n"
        release_notes += "**Installation**: See [SETUP_GUIDE.md](https://github.com/3C-SCSU/Avatar/blob/main/SETUP_GUIDE.md)\n"
